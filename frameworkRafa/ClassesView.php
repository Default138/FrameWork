...
private $caminho = "sistema/view/";
    ...
    function criaLista() {
            ...
            foreach ($listaEntidades as $entidade) {
                ...
                $colunas = "";
                foreach ($listaAtributos as $key => $atributo) {
                    $colunas .= "<td><?php echo \${$entidade}['$key']; ?></td>\n\t\t\t\t\t";
                }
                ...
                <html>
                    ...
                    <table border="1" cellpadding="10" class="table table-bordered">
                        ...
                        <tbody>
                        <?php if (!empty(\$lista{$nomeClasse})): ?>
                            <?php foreach (\$lista{$nomeClasse} as \${$entidade}): ?>
                            <tr>
                                $colunas
                            </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="<?php echo count(\$lista{$nomeClasse}[0] ?? []); ?>">Nenhum registro encontrado.</td>
                            </tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
