// LeitorSQL.php
<?php
ini_set('display_errors', 1); // ALTERADO (16/06): exibição de erros para depuração
    ...
    private function processarTabelas(): void
    {
        ...
        foreach ($matches as $match) {
            ...
            foreach ($linhas as $linha) {
                ...
                $nomeCampo = $campo[1] ?? '';
                $tipoCampo = $campo[2] ?? '';
                if ($nomeCampo === '') continue; // ALTERADO (16/06): ignora linhas sem nome de campo válido
                $nullable = !str_contains($linha, 'NOT NULL'); // ALTERADO (16/06): detecta se o campo aceita nulo
