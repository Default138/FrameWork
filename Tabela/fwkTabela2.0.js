...
for (let tabela of todasTabelas) {
    ...
    if (!qtdLinhas || !qtdColunas) {
        console.log("Erro: É necessário definir linha e coluna");
        continue;
    }
    ...
    for (let expand of tagsExpand) {
        ...
        if (tipoExp === "coluna" && colunaExp + tamanhoExp > qtdColunas) {
            console.log("Erro: Expansão de coluna ultrapassa os limites");
            continue;
        }
        
        if (tipoExp === "linha" && linhaExp + tamanhoExp > qtdLinhas) {
            console.log("Erro: Expansão de linha ultrapassa os limites");
            continue;
        }
    ...
    if (dadosPlanos.length > qtdLinhas * qtdColunas) {
        console.log("Erro: Mais dados do que espaços na tabela");
        continue;
    }
